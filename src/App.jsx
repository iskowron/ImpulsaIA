  ...
  const handle = async () => {
    ...
    try {
      if (mode === "login") {
        ...
        if (error) throw error;
        // ── Siempre requerir confirmación de email antes de poder ingresar.
        // Aunque Supabase cree sesión automáticamente, NO llamamos onAuth():
        // el usuario debe verificar su casilla primero.
  ...
  const authFormJSX = (
    <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 24, padding: "32px 28px", boxShadow: "0 40px 80px rgba(0,0,0,0.5)" }}>
      ...
      <form onSubmit={e => { e.preventDefault(); handle(); }} noValidate>
        ...
        <button type="submit" className="ll-btn ll-btn-primary" disabled={loading || !email || !password}>
      ...
      .then(async ({ data, error }) => {
        if (data) {
          ...
          if (insertError) {
            ...
            // Usar defaults en memoria igual — la app funciona sin DB
                  ...
                  <button onClick={async (e) => {
                    ...
                    try {
                      ...
                      if (data.url) {
                        ...
                        // Mostrar error real de la API para poder debuggear
                        const errorDetail = data.error || data.detail || "Respuesta inesperada del servidor";
                        console.error("[MP Checkout] Error:", res.status, data);
                        alert("Error al iniciar el pago: " + errorDetail + "\n\nSi el problema persiste escribinos a soporte@latinalabs.app");
                        btn.disabled = false;
                        btn.textContent = originalText;
                      }
                    } catch (e) {
                      console.error("[MP Checkout] Excepción:", e);
                      alert("Error de conexion. Verificá tu internet e intentá nuevamente.");
                      btn.disabled = false;
                      btn.textContent = originalText;
                    }
                  }}
