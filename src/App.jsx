...
export default function LatinLabs() {
  ...
  const generate = async () => {
    ...
    if (demoMode) {
      if (demoUsed >= 2) {
        setDemoMode(false);
        localStorage.removeItem("ll_demo_used");
        return;
      }
    }
    ...
    try {
      ...
      if (demoMode) {
        ...
        if (newCount >= 2) {
          setTimeout(() => { setDemoMode(false); localStorage.removeItem("ll_demo_used"); }, 4000);
        }
      }
  ...
  if (!user && !demoMode) return <AuthScreen onAuth={setUser} onDemo={() => { setDemoMode(true); setDemoUsed(0); localStorage.removeItem("ll_demo_used"); }} />;
      ...
      {demoMode && (
        <div style={{ background: "linear-gradient(135deg,rgba(245,158,11,0.12),rgba(249,115,22,0.08))", borderBottom: "1px solid rgba(245,158,11,0.2)", padding: "10px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontSize: 12, color: "#64748b" }}>— {Math.max(0, 2 - demoUsed)} generacion{Math.max(0, 2 - demoUsed) !== 1 ? "es" : ""} gratis restante{Math.max(0, 2 - demoUsed) !== 1 ? "s" : ""}</span>
          </div>
