      ...
      {activeTab === "plans" && (
        <div style={{ maxWidth: 920, margin: "0 auto", padding: "28px 20px", animation: "ll-fade 0.3s ease" }}>
                ...
                {p.disabled ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    <button onClick={async () => {
                      const res = await fetch("/api/stripe-checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ plan: p.name.toLowerCase(), userId: user.id, userEmail: user.email }) });
                      const data = await res.json();
                      if (data.url) window.location.href = data.url;
                    }}
                      style={{ width: "100%", padding: "9px", background: p.popular ? "linear-gradient(135deg,#f59e0b,#f97316)" : "transparent", border: p.popular ? "none" : "1px solid rgba(255,255,255,0.08)", borderRadius: 9, color: p.popular ? "#060609" : "#94a3b8", fontWeight: 600, fontSize: 11, cursor: "pointer", fontFamily: "sans-serif" }}>
                      Pagar con tarjeta
                    </button>
                    <button onClick={async () => {
                      ...
                      style={{ width: "100%", padding: "9px", background: "rgba(0,158,227,0.06)", border: "1px solid rgba(0,158,227,0.2)", borderRadius: 9, color: "#38bdf8", fontWeight: 600, fontSize: 11, cursor: "pointer", fontFamily: "sans-serif" }}>
                      MercadoPago
                    </button>
                  </div>
                )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginTop: 20 }}>
            {[["🔐","Pagos encriptados","Procesado por Stripe y MercadoPago"], ["📊","Sin permanencia","Cancelas en cualquier momento"], ["💳","Multiples metodos","Tarjeta, debito y mas opciones"]].map(([icon, title, desc], i) => (
