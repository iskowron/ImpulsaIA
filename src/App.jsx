...
const ROLES_SEG = [
  { id: "emprendedor", label: "Emprendedor", icon: "🏪", desc: "Resultados rapidos y lenguaje simple" },
  { id: "marketing", label: "Marketing", icon: "📊", desc: "Campanas, brand y engagement" },
  { id: "ventas", label: "Ventas", icon: "💰", desc: "Conversion, urgencia y CTAs" },
];

const ROLE_CONTEXT = {
  emprendedor: "El contenido es para un emprendedor independiente. Usa lenguaje simple, directo y humano. Enfocate en resultados practicos e inmediatos. Voz cercana y autentica, sin tecnicismos. El emprendedor necesita que funcione hoy.",
  marketing: "El contenido es para un equipo de marketing profesional. Enfocate en brand awareness, engagement, metricas de conversion y campanas estrategicas. Usa terminologia de marketing. El objetivo es construir marca y maximizar alcance organico.",
  ventas: "El contenido es para cerrar ventas. Usa lenguaje persuasivo con urgencia real. Incluye manejo de objeciones implicito, prueba social y CTAs de accion inmediata. Cada palabra debe empujar hacia la decision de compra.",
};
...
function AuthScreen({ onAuth, onDemo, startMode = "landing" }) {
  const [mode, setMode] = useState(startMode); // landing | login | register
  ...
  // ── SUBSCRIBE FORM — captura de email ─────────────────────────────────────
  ...
  const handleSubscribe = async (e) => {
    ...
    try {
      const res = await fetch("/api/subscribe", {
        ...
        body: JSON.stringify({ email: subEmail.trim().toLowerCase(), source: "home" }),
      });
      ...
      setSubError("No pudimos guardar tu email. Intentá de nuevo.");
    }
    setSubLoading(false);
  };
  ...
  const SubscribeForm = () => (
    <div style={{ maxWidth: 480, margin: "0 auto" }}>
      {subDone ? (
        <div style={{ background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.25)", borderRadius: 14, padding: "20px 24px", textAlign: "center", animation: "ll-fade 0.4s ease" }}>
          <div style={{ fontSize: 28, marginBottom: 8 }}>✅</div>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#10b981", marginBottom: 6 }}>¡Listo! Revisá tu email.</div>
          <div style={{ fontSize: 13, color: "#94a3b8" }}>Te mandamos las 50 plantillas + el link de la comunidad de WhatsApp.</div>
        </div>
      ) : (
        <form onSubmit={handleSubscribe} style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <input
            ...
            required
            disabled={subLoading}
            style={{ flex: "1 1 220px", background: "rgba(255,255,255,0.04)", border: "1.5px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: "13px 16px", color: "#f8fafc", fontSize: 14, fontFamily: "'Inter',sans-serif", outline: "none", transition: "all 0.2s" }}
            onFocus={e => e.target.style.borderColor = "rgba(245,158,11,0.5)"}
            onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.1)"}
          />
          <button type="submit" disabled={subLoading || !subEmail}
            style={{ background: subLoading || !subEmail ? "rgba(245,158,11,0.4)" : "linear-gradient(135deg,#f59e0b,#f97316)", border: "none", borderRadius: 12, padding: "13px 22px", color: "#0a0a0f", fontSize: 14, fontWeight: 700, fontFamily: "'Inter',sans-serif", cursor: subLoading || !subEmail ? "not-allowed" : "pointer", boxShadow: !subLoading && subEmail ? "0 8px 24px rgba(245,158,11,0.3)" : "none", transition: "all 0.2s", whiteSpace: "nowrap" }}>
            {subLoading ? "Enviando..." : "Quiero las plantillas →"}
          </button>
          {subError && <div style={{ width: "100%", fontSize: 12, color: "#fca5a5", marginTop: 4, textAlign: "center" }}>⚠ {subError}</div>}
        </form>
      )}
    </div>
  );

  // ── LANDING PAGE — CAPTURA DE EMAIL ───────────────────────────────────────
  if (mode === "landing") return (
    <div style={{ minHeight: "100vh", background: "#050508", fontFamily: "'Inter',sans-serif", color: "#e2e8f0", overflow: "hidden" }}>
      <style>{GLOBAL_CSS}</style>

      {/* Background ambient */}
      <div style={{ position: "fixed", inset: 0, backgroundImage: "radial-gradient(rgba(245,158,11,0.04) 1px, transparent 1px)", backgroundSize: "44px 44px", pointerEvents: "none" }} />
      <div style={{ position: "fixed", top: -300, left: -200, width: 700, height: 700, borderRadius: "50%", background: "radial-gradient(circle, rgba(245,158,11,0.08) 0%, transparent 65%)", pointerEvents: "none", animation: "ll-pulse 10s ease-in-out infinite" }} />
      <div style={{ position: "fixed", bottom: -200, right: -150, width: 500, height: 500, borderRadius: "50%", background: "radial-gradient(circle, rgba(249,115,22,0.06) 0%, transparent 70%)", pointerEvents: "none", animation: "ll-pulse 12s ease-in-out infinite reverse" }} />

      {/* NAV */}
      <nav style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(5,5,8,0.92)", backdropFilter: "blur(24px)", borderBottom: "1px solid rgba(255,255,255,0.06)", padding: "14px 28px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
          <span onClick={() => document.getElementById("recursos")?.scrollIntoView({ behavior: "smooth" })} style={{ fontSize: 13, color: "#64748b", cursor: "pointer", fontWeight: 500 }}>Recursos</span>
          <button onClick={() => setMode("login")} style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 9, padding: "7px 16px", cursor: "pointer", fontSize: 13, color: "#94a3b8", fontFamily: "'Inter',sans-serif", fontWeight: 500 }}>Entrar</button>
        </div>
      </nav>

      {/* HERO */}
      <section style={{ maxWidth: 720, margin: "0 auto", padding: "70px 24px 50px", textAlign: "center", position: "relative", zIndex: 1 }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 99, padding: "6px 16px", fontSize: 12, color: "#f59e0b", fontWeight: 600, letterSpacing: "0.03em", marginBottom: 28 }}>
          <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#10b981", boxShadow: "0 0 8px #10b981" }} />
          +1.247 emprendedores activos
        </div>

        <h1 style={{ fontSize: "clamp(36px,6vw,62px)", fontWeight: 900, letterSpacing: "-0.04em", lineHeight: 1.05, marginBottom: 22, color: "#f8fafc" }}>
          Vendé más por WhatsApp con<br />
          <span className="ll-gradient-text">50 plantillas + IA</span> que las personaliza
        </h1>

        <p style={{ fontSize: 17, color: "#94a3b8", maxWidth: 560, margin: "0 auto 36px", lineHeight: 1.6 }}>
          Captación, cierre de ventas y postventa. Listas para copiar, pegar y adaptarlas a TU rubro con inteligencia artificial. Sin teoría, todo aplicable hoy.
        </p>

        <SubscribeForm />

        <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap", marginTop: 18, fontSize: 12, color: "#64748b" }}>
          <span style={{ display: "flex", alignItems: "center", gap: 5 }}><span style={{ color: "#10b981", fontWeight: 700 }}>✓</span> 100% gratis</span>
          <span style={{ display: "flex", alignItems: "center", gap: 5 }}><span style={{ color: "#10b981", fontWeight: 700 }}>✓</span> Sin tarjeta</span>
          <span style={{ display: "flex", alignItems: "center", gap: 5 }}><span style={{ color: "#10b981", fontWeight: 700 }}>✓</span> Acceso instantáneo</span>
        </div>

        <div style={{ marginTop: 36, paddingTop: 28, borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "center", gap: 12, flexWrap: "wrap" }}>
          <div style={{ display: "flex" }}>
            {[
              { c: "linear-gradient(135deg,#f59e0b,#f97316)", l: "M" },
              { c: "linear-gradient(135deg,#f97316,#dc2626)", l: "A" },
              { c: "linear-gradient(135deg,#a78bfa,#8b5cf6)", l: "L" },
              { c: "linear-gradient(135deg,#06b6d4,#3b82f6)", l: "+" },
            ].map((a, i) => (
              <div key={i} style={{ width: 28, height: 28, borderRadius: "50%", background: a.c, border: "2px solid #050508", marginLeft: i === 0 ? 0 : -8, fontSize: 11, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700 }}>{a.l}</div>
            ))}
          </div>
          <div style={{ fontSize: 13, color: "#94a3b8" }}>
            <span style={{ color: "#f59e0b", fontWeight: 700 }}>★★★★★ 4.8/5</span> según +200 emprendedores LATAM
          </div>
        </div>
      </section>

      {/* QUE INCLUYE */}
      <section id="recursos" style={{ maxWidth: 1000, margin: "0 auto", padding: "60px 24px", position: "relative", zIndex: 1 }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          <div style={{ fontSize: 11, color: "#f59e0b", letterSpacing: "0.2em", fontWeight: 700, textTransform: "uppercase", marginBottom: 10 }}>Qué incluye el acceso gratis</div>
          <h2 style={{ fontSize: 32, fontWeight: 900, letterSpacing: "-0.04em", color: "#f8fafc" }}>Todo lo que necesitas para vender más, en un solo lugar</h2>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 16 }}>
          {[
            { icon: "📱", title: "50 plantillas listas", desc: "Captación, cierre de ventas, postventa, promociones y manejo de objeciones — listas para copiar y pegar" },
            { icon: "⚡", title: "IA que personaliza", desc: "10 generaciones gratis cada mes para adaptar las plantillas al tono exacto de tu rubro" },
            { icon: "💬", title: "Comunidad WhatsApp", desc: "Acceso al canal exclusivo con tips semanales, casos reales y recursos que solo compartimos ahí" },
          ].map((b, i) => (
            <div key={i} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16, padding: "24px 22px", transition: "all 0.2s" }}
              onMouseOver={e => { e.currentTarget.style.borderColor = "rgba(245,158,11,0.3)"; e.currentTarget.style.background = "rgba(255,255,255,0.05)"; }}
              ...
              <div style={{ width: 40, height: 40, background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, marginBottom: 14 }}>{b.icon}</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: "#f1f5f9", marginBottom: 8 }}>{b.title}</div>
      ...
      {/* PREVIEW PLANTILLAS */}
      <section style={{ maxWidth: 720, margin: "0 auto", padding: "20px 24px 60px", position: "relative", zIndex: 1 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ fontSize: 11, color: "#f59e0b", letterSpacing: "0.2em", fontWeight: 700, textTransform: "uppercase", marginBottom: 10 }}>Vista previa</div>
          <h2 style={{ fontSize: 28, fontWeight: 900, letterSpacing: "-0.04em", color: "#f8fafc" }}>Algunas plantillas que vas a recibir</h2>
        </div>
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: 24 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#f1f5f9" }}>📂 Captación de clientes nuevos</div>
            <div style={{ fontSize: 11, color: "#10b981", background: "rgba(16,185,129,0.1)", padding: "4px 10px", borderRadius: 99, fontWeight: 600 }}>10 plantillas</div>
          </div>
          <div style={{ display: "grid", gap: 10 }}>
            {[
              "Mensaje de bienvenida automático que duplica respuestas",
              "Cómo abordar a un cliente que dejó de responder",
              "Reactivación después de 30 días sin contacto",
              "Respuesta a consulta de precio sin perder al cliente",
              "...y 46 plantillas más en el PDF completo",
            ].map((t, i) => (
              <div key={i} style={{ background: "rgba(255,255,255,0.02)", borderLeft: "2px solid #f59e0b", padding: "11px 14px", fontSize: 13, color: i === 4 ? "#64748b" : "#cbd5e1", lineHeight: 1.5, fontStyle: i === 4 ? "italic" : "normal" }}>
                {i < 4 && <span style={{ color: "#f59e0b", fontWeight: 700, marginRight: 8 }}>{String(i + 1).padStart(2, "0")}.</span>}
                {t}
              </div>
            ))}
          </div>
      ...
      {/* TESTIMONIO */}
      <section style={{ maxWidth: 720, margin: "0 auto", padding: "0 24px 60px", position: "relative", zIndex: 1 }}>
        <div style={{ background: "rgba(245,158,11,0.04)", border: "1px solid rgba(245,158,11,0.15)", borderRadius: 16, padding: "28px 32px" }}>
          <div style={{ fontSize: 16, color: "#cbd5e1", lineHeight: 1.7, fontStyle: "italic", marginBottom: 18 }}>
            "Apliqué 3 plantillas la primera semana y cerré 2 ventas que estaban frenadas. Mi engagement por WhatsApp creció 40%."
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#e2e8f0" }}>María G.</div>
              <div style={{ fontSize: 11, color: "#64748b" }}>Tienda de ropa · Buenos Aires</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* CTA FINAL */}
      <section style={{ textAlign: "center", padding: "20px 24px 80px", maxWidth: 600, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <h2 style={{ fontSize: 32, fontWeight: 900, letterSpacing: "-0.04em", color: "#f8fafc", marginBottom: 14 }}>
          Empezá hoy. <span className="ll-gradient-text">Es gratis.</span>
        </h2>
        <p style={{ fontSize: 15, color: "#64748b", marginBottom: 28 }}>Sumate a +1.247 emprendedores que ya están vendiendo más con WhatsApp.</p>
        <SubscribeForm />
      </section>

      {/* FOOTER */}
      <footer style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "24px 28px", textAlign: "center", position: "relative", zIndex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 20, marginBottom: 12, flexWrap: "wrap" }}>
          <a href="https://www.instagram.com/latinalabs" target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 6, textDecoration: "none", color: "#475569", fontSize: 12 }}>
          ...
          <a href="https://vm.tiktok.com/ZS9LSn5dM9HQQ-zqXf3/" target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 6, textDecoration: "none", color: "#475569", fontSize: 12 }}>
          ...
          <a href="mailto:hola@latinalabs.app" style={{ color: "#475569", fontSize: 12, textDecoration: "none" }}>hola@latinalabs.app</a>
        </div>
        <p style={{ fontSize: 11, color: "#334155" }}>LatinLabs · Marketing digital para emprendedores LATAM</p>
      </footer>
    </div>
  );
...
export default function LatinLabs() {
  ...
  const [authStartMode, setAuthStartMode] = useState("landing");
  ...
  // Realtime admin subscription — se activa solo cuando el tab admin está abierto
  useEffect(() => {
  ...
  const generate = async () => {
    ...
    // Demo: bloquear silenciosamente si ya usó las 2 generaciones
    if (demoMode && demoUsed >= 2) return;
    // Usuario real: verificar límite
    if (!demoMode && profile && profile.generaciones_usadas >= profile.generaciones_limite) {
      setActiveTab("plans"); return;
    }
    setLoading(true); setResult(""); setQuality(null);
    try {
      const roleCtx = ROLE_CONTEXT[selectedRole] || ROLE_CONTEXT.emprendedor;
      const text = await callAPI(selectedType.prompt(business, topic, selectedRubro, phone || ""), "Sos un experto en marketing digital para negocios latinoamericanos. " + roleCtx + " Respondés directamente con el contenido solicitado, sin explicaciones previas ni comentarios adicionales.");
      setResult(text);
      if (demoMode) {
        // Incrementar contador — NO redirigir, el usuario ve su resultado
        setDemoUsed(prev => prev + 1);
        // No llamar a analyzeQuality en demo (evita llamada extra a la API)
      } else {
        ...
        if (profile) {
          ...
          if (user?.id) await supabase.from("profiles").update({ generaciones_usadas: newCount }).eq("id", user.id);
          setProfile(prev => ({ ...prev, generaciones_usadas: newCount }));
        }
        analyzeQuality(text);
      }
    } catch (e) { setResult("Error al generar. Por favor intentá de nuevo."); }
    setLoading(false);
  };
  ...
  const generateCalendar = async () => {
    ...
    try {
      const roleCtxCal = ROLE_CONTEXT[selectedRole] || ROLE_CONTEXT.emprendedor;
      const text = await callAPI("Crea un calendario de contenido de " + calDays + " dias para " + business + " con tema central: " + topic + ". " + (RUBRO_TONE[selectedRubro] || "") + ". Para cada dia: Dia X - Formato: [red social] - Tema: [tema especifico] - Idea: [descripcion 20 palabras max]. Solo el calendario.", "Sos un experto en marketing digital para negocios latinoamericanos. " + roleCtxCal + " Respondés directamente con el calendario solicitado.");
      setCalResult(text);
  ...
  const saveRubro = async (rubro) => {
    ...
    if (profile) {
      if (user?.id) await supabase.from("profiles").update({ rubro_configurado: rubro }).eq("id", user.id);
  ...
  if (!user && !demoMode) return <AuthScreen onAuth={setUser} startMode={authStartMode} onDemo={() => { setDemoMode(true); setDemoUsed(0); setAuthStartMode("landing"); }} />;
      ...
      {demoMode && (
        <div style={{ background: "linear-gradient(135deg,rgba(245,158,11,0.12),rgba(249,115,22,0.08))", borderBottom: "1px solid rgba(245,158,11,0.2)", padding: "10px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontSize: 12, color: "#64748b" }}>— {Math.max(0, 2 - demoUsed) > 0 ? `${2 - demoUsed} generaci${2 - demoUsed === 1 ? "ón" : "ones"} gratis restante${2 - demoUsed === 1 ? "" : "s"}` : "¡Registrate para seguir generando!"}</span>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => { setAuthStartMode("register"); setDemoMode(false); }} style={{ background: "linear-gradient(135deg,#f59e0b,#f97316)", border: "none", borderRadius: 8, padding: "6px 16px", cursor: "pointer", fontSize: 12, fontWeight: 700, color: "#0a0a0f", fontFamily: "'Inter',sans-serif" }}>
              Crear cuenta gratis →
            </button>
          </div>
        </div>
        ...
        <div className="ll-section-pad" style={{ maxWidth: 900, margin: "0 auto", animation: "ll-fade 0.4s ease" }}>
          ...
          <div className="ll-card" style={{ padding: "28px", marginBottom: 20 }}>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: "block", fontSize: 11, color: "#334155", marginBottom: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}>Soy un</label>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
                {ROLES_SEG.map(role => (
                  <button key={role.id} onClick={() => setSelectedRole(role.id)}
                    style={{ padding: "10px 8px", borderRadius: 12, border: selectedRole === role.id ? "1.5px solid rgba(245,158,11,0.6)" : "1.5px solid rgba(255,255,255,0.07)", background: selectedRole === role.id ? "rgba(245,158,11,0.08)" : "rgba(255,255,255,0.02)", cursor: "pointer", fontFamily: "'Inter',sans-serif", transition: "all 0.15s", textAlign: "center" }}>
                    <div style={{ fontSize: 20, marginBottom: 4 }}>{role.icon}</div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: selectedRole === role.id ? "#f59e0b" : "#475569", letterSpacing: "0.05em" }}>{role.label}</div>
                    <div style={{ fontSize: 10, color: "#334155", marginTop: 2, lineHeight: 1.3 }}>{role.desc}</div>
                  </button>
                ))}
              </div>
            </div>
            ...
            { demoMode && demoUsed >= 2 ? (
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 13, color: "#f59e0b", fontWeight: 600, marginBottom: 10 }}>
                  ⚡ Usaste tus 2 generaciones gratuitas
                </div>
                <button onClick={() => { setAuthStartMode("register"); setDemoMode(false); }}
                  style={{ width: "100%", padding: "14px", background: "linear-gradient(135deg,#f59e0b,#f97316)", border: "none", borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: "pointer", color: "#0a0a0f", fontFamily: "'Inter',sans-serif", boxShadow: "0 8px 32px rgba(245,158,11,0.25)" }}>
                  Crear cuenta gratis para continuar →
                </button>
                <div style={{ fontSize: 11, color: "#334155", marginTop: 8 }}>Sin tarjeta de crédito · 10 generaciones gratis</div>
              </div>
            ) : (
              ...
              <div style={{ marginTop: 20, animation: "ll-fade 0.3s ease" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{ fontSize: 10, background: "rgba(245,158,11,0.1)", color: "#f59e0b", padding: "2px 8px", borderRadius: 99, fontWeight: 600 }}>
                      {ROLES_SEG.find(r => r.id === selectedRole)?.icon} {ROLES_SEG.find(r => r.id === selectedRole)?.label}
                    </span>
                  </div>
