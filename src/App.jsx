        ...
        <div style={{ width: "100%", maxWidth: 400, animation: "ll-fade 0.6s ease" }}>
          ...
          <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 24, padding: "32px 28px", boxShadow: "0 40px 80px rgba(0,0,0,0.5)" }}>
            ...
            <button onClick={handleGoogle} disabled={googleLoading || loading}
              ...
              {googleLoading ? (
                <div style={{ width: 18, height: 18, border: "2px solid rgba(255,255,255,0.2)", borderTopColor: "#f8fafc", borderRadius: "50%", animation: "ll-spin 0.7s linear infinite" }} />
              ) : (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
              ...
              <span style={{ fontSize: 11, color: "#334155", fontWeight: 500, letterSpacing: "0.05em" }}>O con tu email</span>
              <div style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.07)" }} />
            </div>
  ...
  const loadAdmin = async () => {
    ...
    try {
      const [profilesRes, vercelRes] = await Promise.all([
        supabase
          .from("profiles")
          .select("id, email, plan, generaciones_usadas, generaciones_limite, subscription_status, payment_provider, created_at, rubro_configurado")
          .order("created_at", { ascending: false }),
        fetch("/api/admin-stats").then(r => r.json()).catch(() => ({ vercel: null })),
      ]);
      ...
      if (profiles) {
        ...
        const porRubro = {};
        profiles.forEach(p => {
          const r = p.rubro_configurado || "otro";
                ...
                {[
                  ...
                  <div key={i} className="ll-card" style={{ padding: "18px 16px", borderRadius: 16 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                      <span style={{ fontSize: 22 }} dangerouslySetInnerHTML={{__html: k.icon}} />
