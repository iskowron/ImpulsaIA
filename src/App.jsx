  ...
  const handle = async () => {
    ...
    try {
      ...
      setError(
        ...
        e.message === "User already registered" ? "Ya existe una cuenta con ese email." :
        e.message
      );
    }
    setLoading(false);
            ...
            {["login", "register"].map(m => (
              <button key={m} onClick={() => { setMode(m); setError(""); setSuccess(""); }}
                style={{
                  ...
                  fontFamily: "sans-serif", fontSize: 13, fontWeight: 600, transition: "all 0.2s",
...
export default function ImpulsaIA() {
  ...
  const generate = async () => {
    ...
    if (profile && profile.generaciones_usadas >= profile.generaciones_limite) {
      setResult("Alcanzaste el limite de tu plan. Upgrade para continuar generando contenido.");
      return;
    }
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({
          ...
          prompt: selectedType.prompt(business, topic),
        }),
      });
      const data = await res.json();
      const text = data.result || "Error al generar.";
      setResult(text);
      ...
      setResult("Error de conexion. Verifica tu internet e intenta de nuevo.");
    }
    setLoading(false);
  };
  ...
  const barColor = genPct > 80 ? "#ef4444" : "linear-gradient(90deg,#f59e0b,#f97316)";
  const tabBorderActive = "2px solid #f59e0b";
  const tabBorderInactive = "2px solid transparent";

  return (
    <div style={{ fontFamily: "Georgia, serif", minHeight: "100vh", background: "#0a0a0f", color: "#f0ece0" }}>

      <header style={{ borderBottom: "1px solid #1e1e2e", padding: "0 20px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60, position: "sticky", top: 0, zIndex: 100, background: "#0a0a0f", backdropFilter: "blur(12px)" }}>
        ...
        <nav style={{ display: "flex", gap: 2 }}>
          {["generator", "history", "plans"].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              style={{ background: "none", border: "none", borderBottom: activeTab === tab ? tabBorderActive : tabBorderInactive, padding: "6px 14px", cursor: "pointer", fontSize: 12, fontFamily: "sans-serif", color: activeTab === tab ? "#f59e0b" : "#666", transition: "all 0.2s" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ width: 80, height: 4, background: "#1e1e2e", borderRadius: 99 }}>
              <div style={{ width: genPct + "%", height: "100%", background: barColor, borderRadius: 99, transition: "width 0.4s" }} />
            </div>
          </div>
      ...
      {activeTab === "generator" && (
        <div style={{ maxWidth: 860, margin: "0 auto", padding: "36px 20px" }}>
          <div className="fade-in" style={{ textAlign: "center", marginBottom: 36 }}>
            ...
            <p style={{ fontFamily: "sans-serif", color: "#666", fontSize: 14 }}>Hola {user.email.split("@")[0]} - que contenido necesitas hoy?</p>
          </div>
          ...
          <div style={{ background: "#12121a", border: "1px solid #1e1e2e", borderRadius: 18, padding: "28px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 20 }}>
              ...
              <div>
                <label style={{ display: "block", fontFamily: "sans-serif", fontSize: 11, color: "#888", marginBottom: 6, letterSpacing: "0.08em", textTransform: "uppercase" }}>Tema o campana</label>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
                {CONTENT_TYPES.map(ct => (
                  <button key={ct.id} onClick={() => setSelectedType(ct)}
                    style={{
                      background: selectedType.id === ct.id ? "rgba(245,158,11,0.1)" : "none",
                      border: selectedType.id === ct.id ? "1px solid rgba(245,158,11,0.4)" : "1px solid #2a2a3a",
            ...
            {genUsadas >= genLimite && (
              <div style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 10, padding: "12px 16px", marginBottom: 16, fontFamily: "sans-serif", fontSize: 13, color: "#fca5a5", textAlign: "center" }}>
                Alcanzaste tu limite. <span onClick={() => setActiveTab("plans")} style={{ color: "#f59e0b", cursor: "pointer", fontWeight: 700 }}>Upgradear plan</span>
              </div>
            )}
            ...
            {result && (
              <div style={{ marginTop: 20 }}>
                ...
                <div style={{ background: "#0a0a0f", border: "1px solid rgba(245,158,11,0.15)", borderRadius: 12, padding: "18px 20px", fontFamily: "sans-serif", fontSize: 13, lineHeight: 1.8, color: "#ddd", whiteSpace: "pre-wrap", maxHeight: 300, overflowY: "auto" }}>
      ...
      {activeTab === "history" && (
        <div style={{ maxWidth: 740, margin: "0 auto", padding: "36px 20px" }}>
          ...
          <p style={{ fontFamily: "sans-serif", color: "#666", fontSize: 13, marginBottom: 24 }}>Tus ultimas generaciones de esta sesion</p>
          {history.length === 0 ? (
            <div style={{ background: "#12121a", border: "1px solid #1e1e2e", borderRadius: 16, padding: 40, textAlign: "center" }}>
              <div style={{ fontSize: 40, marginBottom: 12 }}>📋</div>
              <div style={{ fontFamily: "sans-serif", color: "#666", fontSize: 14 }}>Aun no generaste contenido en esta sesion</div>
            ...
            <div key={i} className="fade-in" style={{ background: "#12121a", border: "1px solid #1e1e2e", borderRadius: 12, padding: 18, marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <span style={{ background: "rgba(245,158,11,0.15)", color: "#f59e0b", padding: "2px 10px", borderRadius: 999, fontSize: 10, fontFamily: "sans-serif", fontWeight: 700 }}>{h.type}</span>
      ...
      {activeTab === "plans" && (
        <div style={{ maxWidth: 860, margin: "0 auto", padding: "36px 20px" }}>
          <div style={{ textAlign: "center", marginBottom: 40 }}>
            ...
            <p style={{ fontFamily: "sans-serif", color: "#666", fontSize: 14 }}>Todos los planes incluyen 7 dias de prueba gratis.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>
            {PLANS.map((p, i) => (
              <div key={i} className="fade-in" style={{ background: "#12121a", border: p.popular ? "1px solid #f97316" : "1px solid #1e1e2e", borderRadius: 18, padding: 24, position: "relative" }}>
                {p.popular && <div style={{ position: "absolute", top: -11, left: "50%", transform: "translateX(-50%)", background: "linear-gradient(135deg,#f59e0b,#f97316)", color: "#0a0a0f", fontSize: 9, fontWeight: 800, padding: "3px 12px", fontFamily: "sans-serif", letterSpacing: "0.1em", whiteSpace: "nowrap" }}>MAS POPULAR</div>}
      ...
      <footer style={{ borderTop: "1px solid #1e1e2e", padding: 20, textAlign: "center", marginTop: 40 }}>
        <p style={{ fontFamily: "sans-serif", fontSize: 11, color: "#333" }}>
          ImpulsaIA · Misiones, Argentina · Powered by Claude AI
        </p>
      </footer>
    </div>
  );
