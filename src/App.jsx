...
const TIPS = [
  { icon: "🔥", cat: "Constancia", text: "La consistencia vende mas que la perfeccion. Publica aunque no sea perfecto." },
  { icon: "⏰", cat: "Timing", text: "El 80% del engagement ocurre en las primeras 2 horas. Publica cuando tu audiencia esta activa." },
  { icon: "📈", cat: "Crecimiento", text: "Los negocios que publican 5+ veces por semana crecen 3x mas rapido que los que publican 1 vez." },
  { icon: "💬", cat: "WhatsApp", text: "WhatsApp tiene 98% de tasa de apertura vs 20% del email. Es el canal mas poderoso de LATAM." },
  { icon: "⭕", cat: "Stories", text: "Las stories con preguntas generan 4x mas respuestas. Siempre incluye una interaccion." },
  ...
  { icon: "📅", cat: "Planificacion", text: "Los negocios que planifican su contenido con una semana de anticipacion publican 4x mas." },
  ...
  { icon: "📸", cat: "Visual", text: "Las publicaciones con imagenes reales del negocio generan 3x mas engagement que las genericas." },
  { icon: "🧠", cat: "Psicologia", text: "La gente no compra productos, compra la mejor version de si misma. Vende transformacion." },
  { icon: "💡", cat: "Ideas", text: "Si no sabes que publicar, preguntale a tus clientes que les genera dudas. Cada pregunta es un post." },
  { icon: "🚀", cat: "Lanzamiento", text: "Para lanzar algo nuevo, empieza a generar expectativa 7 dias antes. El hype se construye." },
  { icon: "📊", cat: "Metricas", text: "Mide lo que importa: cuantas consultas genera tu contenido, no solo cuantos likes." },
  { icon: "🌙", cat: "Horarios", text: "En Argentina, los mejores horarios para publicar son 12-13hs y 20-22hs de lunes a viernes." },
  ...
  { icon: "❤️", cat: "Comunidad", text: "Responder comentarios en la primera hora aumenta el alcance organico hasta un 40%." },
  { icon: "🎬", cat: "Video", text: "Los reels y videos cortos tienen 2x mas alcance organico que las imagenes estaticas en 2024." },
...
function RotatingTip() {
  ...
  return (
    <div style={{ padding: "12px 20px", borderBottom: "1px solid #1a1a2e", background: "#0a0a14" }}>
      <div style={{
        ...
        <div style={{ width: 34, height: 34, background: "rgba(245,158,11,0.12)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>
          {tip.icon}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <span style={{ fontSize: 9, color: "#f59e0b", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", marginRight: 8 }}>{tip.cat}</span>
          <span style={{ fontSize: 12, color: "#888", lineHeight: 1.5 }}>{tip.text}</span>
        </div>
        <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
          {TIPS.slice(0, 5).map((_, i) => (
            <div key={i} onClick={() => { setVisible(false); setTimeout(() => { setCurrent(i); setVisible(true); }, 300); }}
              style={{ width: i === current % 5 ? 16 : 5, height: 5, borderRadius: 99, background: i === current % 5 ? "#f59e0b" : "#1a1a2e", cursor: "pointer", transition: "all 0.3s ease" }} />
          ))}
        </div>
      </div>
    </div>
    ...
    <div style={{ minHeight: "100vh", background: "#07070d", display: "flex", fontFamily: "sans-serif" }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32 }}>
        <div style={{ width: "100%", maxWidth: 380 }}>
          ...
          <div style={{ marginTop: 14, background: "rgba(245,158,11,0.05)", border: "1px solid rgba(245,158,11,0.12)", borderRadius: 9, padding: "11px 13px", display: "flex", gap: 10, alignItems: "flex-start" }}>
            <span style={{ fontSize: 18, flexShrink: 0 }}>{tip.icon}</span>
            <div>
              <div style={{ fontSize: 9, color: "#f59e0b", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 4 }}>{tip.cat}</div>
              <div style={{ fontSize: 11, color: "#555", lineHeight: 1.6 }}>{tip.text}</div>
            </div>
          </div>
