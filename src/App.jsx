...
const ADMIN_EMAIL = "skowronignacio@gmail.com";
...
const CONTENT_TYPES = [
  ...
  { id: "instagram", label: "Instagram", icon: "📸", prompt: (b, t, r) => "Crea un post de Instagram para " + b + ". Tema: " + t + ". " + (RUBRO_TONE[r] || "") + ". Texto maximo 150 palabras, CTA poderoso y 10 hashtags. Solo el contenido." },
  { id: "stories", label: "Stories", icon: "⭕", prompt: (b, t, r) => "Crea una secuencia de 5 stories de Instagram para " + b + " sobre: " + t + ". " + (RUBRO_TONE[r] || "") + ". Cada story: texto maximo 15 palabras, sticker sugerido y CTA." },
  ...
  { id: "ads", label: "Ads", icon: "🎯", prompt: (b, t, r) => "Crea un anuncio Facebook/Instagram Ads para " + b + ". Tema: " + t + ". " + (RUBRO_TONE[r] || "") + ". Titular 40 chars, Descripcion 125 chars, Texto 90 palabras y CTA boton." },
  { id: "linkedin", label: "LinkedIn", icon: "💼", prompt: (b, t, r) => "Escribe un post de LinkedIn para " + b + " sobre: " + t + ". Gancho inicial, desarrollo con valor, CTA final. Maximo 200 palabras." },
  { id: "landing", label: "Landing", icon: "🚀", prompt: (b, t, r) => "Crea el copy hero de una landing page para " + b + ". Oferta: " + t + ". " + (RUBRO_TONE[r] || "") + ". H1 8 palabras, H2 15 palabras, 3 beneficios con emoji y CTA boton." },
];
...
const GLOBAL_CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .ll-shimmer-btn{background-size:200% auto;animation:ll-shimmer 3s linear infinite}
  ...
  .ll-btn-ghost:hover{background:rgba(255,255,255,0.08);color:#f8fafc}
  .ll-nav{background:none;border:none;padding:8px 16px;cursor:pointer;font-size:14px;font-weight:500;font-family:'Inter',sans-serif;transition:all 0.15s;border-bottom:2px solid transparent;color:#475569}
  ...
  .ll-tag{font-size:10px;padding:3px 10px;border-radius:99px;font-weight:600;letter-spacing:0.05em}
`;
...
function AuthScreen({ onAuth }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#050508", display: "flex", fontFamily: "'Inter',sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", bottom: -200, right: -100, width: 500, height: 500, borderRadius: "50%", background: "radial-gradient(circle, rgba(249,115,22,0.1) 0%, transparent 70%)", pointerEvents: "none", animation: "ll-pulse 8s ease-in-out infinite reverse" }} />
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 40px", position: "relative", zIndex: 1 }}>
        <div style={{ width: "100%", maxWidth: 400, animation: "ll-fade 0.6s ease" }}>

          {/* Logo */}
          <div style={{ textAlign: "center", marginBottom: 40 }}>
            <div style={{ width: 56, height: 56, background: "linear-gradient(135deg,#f59e0b,#f97316)", borderRadius: 16, display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 26, marginBottom: 20, boxShadow: "0 0 60px rgba(245,158,11,0.4)" }}>⚡</div>
            ...
            <div style={{ fontSize: 16, color: "#475569", fontWeight: 400 }}>Crea contenido que vende, en tu idioma</div>
          </div>
          ...
          <div style={{ textAlign: "center", marginTop: 16, fontSize: 12, color: "#1e293b" }}>
            Sin tarjeta de credito · 10 generaciones gratis al registrarte
          </div>
        </div>
      </div>

      {/* Right — visual panel */}
      <div style={{ width: 480, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", justifyContent: "center" }}>
        {/* Background image */}
        <div style={{ position: "absolute", inset: 0, backgroundImage: "url('https://images.unsplash.com/photo-1552664730-d307ca884978?w=960&q=80')", backgroundSize: "cover", backgroundPosition: "center" }} />
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, rgba(5,5,8,0.92) 0%, rgba(5,5,8,0.75) 50%, rgba(5,5,8,0.88) 100%)" }} />

        <div style={{ position: "relative", zIndex: 1, padding: "48px 40px" }}>
          <div style={{ fontSize: 11, color: "#f59e0b", letterSpacing: "0.2em", fontWeight: 700, textTransform: "uppercase", marginBottom: 24 }}>Por que LatinLabs</div>

          <div style={{ fontSize: 32, fontWeight: 800, color: "#f8fafc", letterSpacing: "-0.04em", lineHeight: 1.2, marginBottom: 32 }}>
            Genera contenido<br />
            <span className="ll-gradient-text">que realmente vende</span>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 32 }}>
            {[
              ...
              <div key={i} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: "16px 14px" }}>
                <div style={{ fontSize: 26, fontWeight: 900, letterSpacing: "-0.04em", marginBottom: 2 }}>
                  <span className="ll-gradient-text">{s.n}</span>
                </div>
          ...
          {[
            ["💬", "WhatsApp primero", "El canal con 98% de tasa de apertura"],
            ["🎯", "Contenido por rubro", "Lenguaje adaptado a tu industria"],
            ["📅", "Plan de contenido", "7 o 30 dias con un clic"],
            ...
            <div key={i} style={{ display: "flex", gap: 14, marginBottom: 18, alignItems: "center" }}>
              <div style={{ width: 36, height: 36, background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>{icon}</div>
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: "#e2e8f0", marginBottom: 1 }}>{title}</div>
          ...
          {/* Social proof */}
          <div style={{ marginTop: 28, background: "rgba(245,158,11,0.06)", border: "1px solid rgba(245,158,11,0.15)", borderRadius: 12, padding: "14px 16px" }}>
            <div style={{ fontSize: 13, color: "#cbd5e1", lineHeight: 1.7, fontStyle: "italic" }}>
              "Con LatinLabs publico todos los dias sin estresarme por que escribir. Mi engagement subio 40% en el primer mes."
            </div>
            <div style={{ fontSize: 11, color: "#475569", marginTop: 8, display: "flex", alignItems: "center", gap: 6 }}>
              <div style={{ width: 24, height: 24, borderRadius: "50%", background: "linear-gradient(135deg,#f59e0b,#f97316)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#0a0a0f" }}>M</div>
              Maria G. · Tienda de ropa online
            </div>
          </div>
        </div>
      </div>
...
export default function LatinLabs() {
  ...
  const loadAdmin = async () => {
    ...
    try {
      ...
      if (profiles) {
        ...
        setAdminData({ total, pagos: pagos.length, nuevosHoy, nuevosSemana, totalGen, mrr, porPlan, recientes: profiles.slice(0, 15) });
      }
  ...
  const tabs = [["generator","Generar"], ["calendar","Calendario"], ["history","Historial"], ["plans","Planes"], ...(isAdmin ? [["admin","⚡ Admin"]] : [])];
  ...
  return (
    <div style={{ fontFamily: "'Inter',sans-serif", minHeight: "100vh", background: "#050508", color: "#e2e8f0" }}>
      ...
      {/* Onboarding */}
      {showOnboarding && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", backdropFilter: "blur(12px)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ background: "#0c0c12", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 24, padding: "36px 32px", maxWidth: 480, width: "100%", boxShadow: "0 60px 120px rgba(0,0,0,0.7)", animation: "ll-fade 0.3s ease" }}>
            <div style={{ textAlign: "center", marginBottom: 28 }}>
              <div style={{ fontSize: 40, marginBottom: 12 }}>👋</div>
              <div style={{ fontSize: 22, fontWeight: 800, color: "#f8fafc", marginBottom: 8, letterSpacing: "-0.03em" }}>Bienvenido a LatinLabs</div>
              <div style={{ fontSize: 14, color: "#475569", lineHeight: 1.7 }}>Contanos a que se dedica tu negocio para personalizar el contenido con el tono justo.</div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {RUBROS.map(r => (
                <button key={r.id} className="ll-rubro ll-card-hover" onClick={() => saveRubro(r.id)} style={{ padding: "12px 14px", borderRadius: 12 }}>
                  <span style={{ fontSize: 18 }}>{r.icon}</span>
                  <span style={{ fontSize: 13, color: "#94a3b8", fontWeight: 500 }}>{r.label}</span>
                </button>
              ))}
            </div>
            <button onClick={() => setShowOnboarding(false)} style={{ width: "100%", marginTop: 16, background: "none", border: "none", color: "#334155", fontSize: 13, cursor: "pointer", padding: "8px", fontFamily: "'Inter',sans-serif" }}>Configurar mas tarde</button>
          </div>
        </div>
      )}

      {/* Header */}
      <header style={{ height: 60, padding: "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100, background: "rgba(5,5,8,0.9)", backdropFilter: "blur(24px)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "6px 12px", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button onClick={() => setShowOnboarding(true)} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 9, padding: "5px 9px", cursor: "pointer", fontSize: 16 }} title="Cambiar rubro">{rubroActual?.icon || "✦"}</button>
          <div style={{ width: 1, height: 22, background: "rgba(255,255,255,0.08)" }} />
          <span style={{ fontSize: 12, color: "#334155", maxWidth: 110, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user.email.split("@")[0]}</span>
      ...
      {/* GENERATOR */}
      {activeTab === "generator" && (
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "36px 24px", animation: "ll-fade 0.4s ease" }}>
          ...
          <div style={{ marginBottom: 32, textAlign: "center" }}>
            ...
            <h1 style={{ fontSize: 40, fontWeight: 900, letterSpacing: "-0.05em", lineHeight: 1.1, marginBottom: 12 }}>
              <span style={{ color: "#f8fafc" }}>Tu negocio merece </span>
              <span className="ll-gradient-text">contenido de calidad</span>
            </h1>
            <p style={{ fontSize: 16, color: "#475569", maxWidth: 500, margin: "0 auto" }}>Genera posts, emails y mensajes listos para publicar en segundos, con el tono exacto de tu industria.</p>
          </div>
      ...
      {/* HISTORY */}
      {activeTab === "history" && (
        <div style={{ maxWidth: 820, margin: "0 auto", padding: "36px 24px", animation: "ll-fade 0.4s ease" }}>
      ...
      {activeTab === "admin" && isAdmin && (
        <div style={{ maxWidth: 980, margin: "0 auto", padding: "36px 24px", animation: "ll-fade 0.4s ease" }}>
          <div style={{ marginBottom: 28 }}>
            <div style={{ fontSize: 11, color: "#f59e0b", letterSpacing: "0.2em", fontWeight: 700, textTransform: "uppercase", marginBottom: 10 }}>Solo visible para vos</div>
            <h1 style={{ fontSize: 36, fontWeight: 900, letterSpacing: "-0.05em", color: "#f8fafc", marginBottom: 6 }}>Panel de administracion</h1>
            <p style={{ fontSize: 15, color: "#475569" }}>Vista en tiempo real del negocio</p>
          </div>

          {adminLoading && <div style={{ textAlign: "center", padding: "60px 0", fontSize: 14, color: "#334155" }}>Cargando datos de Supabase...</div>}

          {adminData && !adminLoading && (
            <>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 16 }}>
                {[
                  { label: "Usuarios totales", value: adminData.total, color: "#f59e0b", icon: "👥" },
                  { label: "Clientes pagos", value: adminData.pagos, color: "#10b981", icon: "💳" },
                  { label: "MRR", value: "USD " + adminData.mrr, color: "#a78bfa", icon: "💰" },
                  { label: "Generaciones totales", value: adminData.totalGen.toLocaleString(), color: "#38bdf8", icon: "⚡" },
                ].map((k, i) => (
                  <div key={i} className="ll-card" style={{ padding: "20px 18px", borderRadius: 16 }}>
                    ...
                    <div style={{ fontSize: 28, fontWeight: 900, color: k.color, letterSpacing: "-0.04em" }}>{k.value}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 20 }}>
                {[
                  ...
                  { label: "Tasa de conversion", value: adminData.total > 0 ? Math.round((adminData.pagos / adminData.total) * 100) + "%" : "0%", color: "#10b981" },
                  { label: "Gen. promedio/usuario", value: adminData.total > 0 ? Math.round(adminData.totalGen / adminData.total) : 0, color: "#38bdf8" },
                ].map((k, i) => (
                  <div key={i} className="ll-card" style={{ padding: "18px", borderRadius: 14 }}>
                    <div style={{ fontSize: 10, color: "#334155", letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 700, marginBottom: 6 }}>{k.label}</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                <div className="ll-card" style={{ padding: "22px", borderRadius: 16 }}>
                  <div style={{ fontSize: 11, color: "#334155", letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 700, marginBottom: 18 }}>Distribucion por plan</div>
                  {[
                    ...
                    <div key={i} style={{ marginBottom: 14 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                        ...
                        <span style={{ fontSize: 14, fontWeight: 700, color: p.color }}>{p.count}</span>
                      </div>
                ...
                <div className="ll-card" style={{ padding: "22px", borderRadius: 16 }}>
                  <div style={{ fontSize: 11, color: "#334155", letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 700, marginBottom: 14 }}>Ultimos usuarios registrados</div>
                  <div style={{ maxHeight: 260, overflowY: "auto" }}>
                    {adminData.recientes.map((u, i) => {
                      ...
                      const fecha = u.created_at ? new Date(u.created_at).toLocaleDateString("es-AR", { day: "2-digit", month: "2-digit" }) : "—";
                      return (
                        <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: i < adminData.recientes.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                          <div>
                            <div style={{ fontSize: 12, color: "#94a3b8", fontWeight: 500 }}>{u.email || "—"}</div>
                            <div style={{ fontSize: 11, color: "#334155", marginTop: 1 }}>{u.generaciones_usadas || 0} gen · {fecha}</div>
                          </div>
                          <span style={{ fontSize: 9, padding: "3px 9px", borderRadius: 99, background: pc + "18", color: pc, fontWeight: 800, textTransform: "uppercase" }}>{pl}</span>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div style={{ marginTop: 14, textAlign: "right" }}>
                <button onClick={() => { setAdminData(null); loadAdmin(); }} style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 9, padding: "7px 16px", color: "#475569", fontSize: 12, cursor: "pointer", fontFamily: "'Inter',sans-serif", fontWeight: 500 }}>
                  Actualizar datos
                </button>
              </div>
            </>
      ...
      <footer style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "20px 28px", textAlign: "center", marginTop: 60 }}>
        <p style={{ fontSize: 12, color: "#1e293b" }}>LatinLabs · Powered by Claude AI</p>
      </footer>
    </div>
  );
}
